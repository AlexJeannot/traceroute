...
void send_datagram(void)
{
    ...
    if ((ret = sendto(env.sendsock, sendbuf, 150, 0, env.targetinfo->ai_addr, (socklen_t)sizeof(env.sendaddr))) < 0)
    {
...
void traceroute_loop(void)
{
    ...
    int val = 1;
    int ret = setsockopt(env.sendsock, IPPROTO_IP, IP_MTU, &val, sizeof(val));
    perror("DONT FRAG: ");
...
void create_socket(void)
{
    ...
    if ((env.sendsock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0)
...

{
    ...
    env.sendaddr.sin_port = htons(33436); //33435
